...
- `smartQueue.ts` - Smart Queue drain driver: dispatches deferred sessions (`status: 'queued'` → `launchPrepared`) and rate-limited turns (`rateLimited` → `continueRateLimited`) when usage windows reset or a custom `targetStartAt` passes; started once via `startSmartQueue()` in the main layout
- `schedules.ts` - Native Scheduling: durable `Schedule` entities (recurring + managed one-shot session starts and in-session messages), `computeNextFire` recurrence math, `schedules.json` persistence, and the `startSchedules()` 30s driver (see Native Scheduling)
...
- `RepositoryView.svelte` - Main-pane repository landing view: metadata, icon/color, launch profiles via `LaunchBar`. "Explore with Claude/Codex" (`utils/repoExplore.ts`) is the single auto-fill path: one visible SDK session explores the repo and returns one JSON block that populates description, keywords, vocabulary, icon, color **and** launch commands/profiles (hand-written commands are preserved; only auto-detected ones are replaced). It also reports a repo-relative path to the project's **own favicon/logo** if it has one (`icon_image` in the JSON) — `utils/repoIconImage.ts` reads it through `read_repo_image` (`image_cmds.rs`, path confined to the repo, png/svg/ico/jpg/webp), rasterizes it to a 96px square PNG data URL and stores it on `RepoConfig.icon_image`. `RepoIcon.svelte` renders that image instead of the curated `icon` when present; the curated icon stays as the fallback (a missing/unusable/non-square file just leaves it in place) and "Drop logo" in Repository Settings clears it. The old headless `generate_launch_profile_with_*` sidecar rail and the native `scan_repo_launch_commands` scan were removed in favour of it
...
- `schedule_cmds.rs` - Native Scheduling persistence (`get_schedules`/`save_schedules`, opaque JSON `schedules.json`)
- `cli_cmds.rs` - `ow` CLI inbox (`take_cli_requests`/`write_cli_ack`/`path_is_dir`), inbox + CLI bin dir paths, and `cli_session_env` (the `OPENWHISPERER_SESSION_ID` / `OPENWHISPERER_INBOX_DIR` / PATH env pairs injected into every agent session)
...
Exhaustion detection lives in `queueDetection.ts` (threshold ≥100% utilization of the 5h or 7d window, per provider — Claude and Codex both) which deliberately imports neither `sdkSessions` nor `smartQueue` to break the import cycle. Draining is FIFO per provider with configurable fuzzy stagger (`settings.queue`, Settings → Smart Queue). UI: `QueueIndicator` (header, also folds in the next native-schedule fire) and `RateLimitBanner` (session). Distinct from `createSessionQueue` in `sessionLaunch.ts`, which is just a sequential batch-launch stagger.

**Dispatch policy (deliberate):** the Smart Queue master toggle and the fuzzy reset delays gate/apply **only to `rate_limit` items** — `scheduled` and `after_sessions` are explicit per-item user actions that always dispatch, undelayed and hoisted ahead of rate-limit items in the drain order. The `scheduled` readiness branch fires purely on `now > targetStartAt` (no exhaustion hold; if the provider rejects, the item re-parks as `rate_limit`). `nextQueueResetAt` reads `targetStartAt ?? resetsAt`. `status: 'queued'` sessions are exempt from the `max_sessions` overflow-archiving sweep (a far-future scheduled session must not be archived away).

## Native Scheduling

Schedule session starts and in-session messages at custom times and on recurring rules — the lightweight, native alternative to Sequences (spec: `docs/scheduling-feature-spec.md`; local PC wall clock is the source of truth, no timezone knob, same policy as the sequences scheduler). Two rails, one user-facing schedule menu:

- **One-shot custom time rides the existing Smart Queue rails.** `startSetupSession`/`launchSession`/`handleSetupSessionStart` accept `schedule: { at: epoch ms }` (alongside the existing `'5h' | '7d' | 'after_sessions'`) → session parks as `status: 'queued'` with `queueInfo = { reason: 'scheduled', targetStartAt: at }` (no `window`). In-session turns use `sdkSessions.queueTurnAtTime(id, prompt, images, at, action?)` → `rateLimited = { reason: 'scheduled', targetStartAt: at }` (no `resetsAt`/`window`) + a ghost message with `queued: 'at_time'`. The Smart Queue's existing `scheduled` branch fires both within ±30s. Custom-time shapes render "Scheduled for tomorrow 09:00"-style labels (`formatScheduleTarget` in `utils/duration.ts`) in `SdkView`'s queued panel, `SessionListItem`'s queue badge, `RateLimitBanner`, and `QueuedTurnGhost`.
- **Recurring (and managed one-shots) are a durable `Schedule` entity** in `src/lib/stores/schedules.ts`, persisted to `schedules.json` (`schedule_cmds.rs`: `get_schedules`/`save_schedules`, opaque JSON, frontend owns the schema, `.dev.json` split — pile pattern). A schedule *spawns* sessions/turns when it fires; it is never consumed. Shape: `target` (`session` — concrete repoId/model/effort/provider/account/worktree, never auto-repo; or `message` — bound sessionId with `ifSessionGone: 'skip' | 'launch_new'` + creation-time fallback snapshot, optional `action: 'compact'`), `when` (`{ kind: 'at' }` or `{ kind: 'recurring', rule }` — daily / weekly with day set + every-N-weeks / monthly day 1..28 or `'last'` / every-N-days, plus `endAt`/`maxRuns`), `waitForIdle`, `catchUp: 'skip' | 'run_once'`, computed+persisted `nextFireAt`, bounded run `history` (20). Pure exported `computeNextFire` does the local-time recurrence math.
- **Driver** (`startSchedules()`, started after `schedules.load()` in the main layout): 30s tick; idle-hold (session targets with `waitForIdle` wait on the repo scope; message targets always wait for their session to be idle); **advance-before-fire, unconditionally** (a failed run never stalls the series); catch-up on load per `catchUp` (missed occurrences coalesce into one `run_once` fire, or `skip` advances with a history entry); same-tick launches go through a staggered `createSessionQueue`. Session launches use `launchSession` with `tag: { schedule }` (→ `SdkSession.scheduleTag`, persisted via `schedule_tag` on `PersistedSdkSession`) and `onWorktreeError: 'fail'` (a new `LaunchSessionOptions` option — unattended runs must never silently fall back to editing the main repo; `'fallback'` remains the default elsewhere). Exhausted-at-fire-time launches park as `rate_limit` and are recorded as `deferred` in history.
- **Schedule notice:** every fired prompt carries a `<system-message>` provenance block (`buildScheduleNotice` in `schedules.ts`) so the agent knows the turn came from a schedule rather than a user typing just now — label, `describeScheduleSpec` pattern, "this is run #N", the previous run's time, and the next fire time; one-shots get only the first two lines. Deliberately **descriptive only** — no behavioural instructions, so scheduled agents behave exactly like interactive ones — and always on (provenance, not a preference). Session targets ride the existing invisible `systemPrompt` rail (appended to Claude's preset / prepended to the first Codex turn, and preserved as `preparedSystemPrompt` when the launch parks in the Smart Queue); message targets have no such rail mid-session, so it rides `sendPrompt`'s `opts.systemNotice` — folded into the sent prompt next to `SCREENSHOT_PROMPT_NOTICE`, never into the transcript message. Compaction turns get none (no prompt to frame).
- **UI:** third sidebar tab **Sessions | Pile | Scheduled** (`ScheduleList` cards: pattern summary via `describeScheduleSpec`, countdown, enable toggle, Run now — fires without advancing the recurrence — and Skip next / Duplicate / Delete) with `ScheduleDetailView` as the main-pane editor (label, prompt, target section, One time ↔ Recurring timing via `ScheduleTimePicker` presets + `RecurrenceEditor`, waitForIdle/catchUp, run history linking to launched sessions). Entry points: the schedule split-button menus in `SdkPromptInput` (live session → "At a time…" = `queueTurnAtTime`, "Recurring…" = message-target schedule) and `SessionSetupView` ("At a time…" = `schedule: { at }`, "Recurring…" = session-target schedule; disabled in auto-repo mode). Components in `src/lib/components/schedule/`.
- **Limits:** nothing fires while the app is fully closed (tray keeps the webview alive — hidden-window timers may be throttled to ~1 min, fine at minute granularity); catch-up runs on next launch. Schedules store text prompts only (no images).

### `ow` CLI (agent-driven scheduling)

A standalone Rust CLI (`cli/`, binary `ow`) that Claude Code / Codex call from inside a session — "schedule a follow-up on this feature tomorrow" — taught by a skill (`cli/skill/SKILL.md`, installed to `~/.claude/skills`, `~/.codex/skills`, `~/.agents/skills` by `ow install-skills`). Spec: `docs/cli-scheduling-spec.md`.

- **Transport is files, not HTTP:** the CLI writes `<config dir>/cli-inbox[-dev]/<id>.request.json` atomically; `stores/cliInbox.ts` polls `take_cli_requests` every 2 s, applies the request through the normal store paths (`schedules.add` / `launchSession`), and answers with `<id>.ack.json` (`cli_cmds.rs`). `schedule` requests left behind while the app was closed are applied on next launch; other kinds expire after 60 s.
- **Session identity:** `create_sdk_session` injects `OPENWHISPERER_SESSION_ID` and `OPENWHISPERER_INBOX_DIR` into every agent session (same env rail as `GH_TOKEN`), and prepends the CLI bin dir (`%LOCALAPPDATA%\OpenWhisperer\bin` / `~/.local/bin`) to PATH when it exists — so `ow` works inside sessions without a terminal restart.
- **Repo/worktree:** the CLI detects the main worktree root (`git rev-parse --git-common-dir`) and the cwd's worktree; the app matches the root against registered repos (`--repo <name>` overrides). Default target is a **new session in the same worktree**, inheriting the invoking session's model and effort; `--same-session` makes a message-target schedule (`ifSessionGone: 'launch_new'`). `ScheduleSessionTarget.worktreePath` (and the message fallback's) is new for this: an existing worktree to run in; **if it is gone at fire time the run falls back to the main checkout** (all schedule kinds — deliberate exception to the "unattended runs never touch main" rule, noted on the run via `ScheduleRun.note`). `waitForIdle` is scoped to that cwd. `Schedule.source: 'cli'` shows a CLI badge in the Scheduled tab.
- **Run now:** `ow run` → `launchSession` directly (no Schedule entity); `ow run --same-session` → a due message-target schedule so it waits for the session to be idle instead of colliding with a running turn.
- **Model picks the provider:** there is no `--provider` flag. `--model` takes a short alias (`opus`, `astra`, `terra`) that `cliInbox.ts` expands via `resolveModelAlias` and validates against the catalog (unknown name / disabled provider → refused in the ack, with the available models listed); `getProviderForModel` then derives the provider. Effort inherits from the invoking session even when the model moves the run to the other provider (clamped by `clampEffortForModel`); an account only inherits within its own provider. The request's `provider` field is retained but deprecated — honored only when no model was requested, so an `ow` binary older than 0.2.0 keeps working.
- **Bundled + one-click install:** `scripts/build-cli.mjs` (`npm run cli:build`, run by `tauri:dev` and `beforeBuildCommand`, honours `TAURI_ENV_TARGET_TRIPLE` for CI cross builds) builds `cli/` and stages `src-tauri/binaries/ow-<triple>[.exe]`, which `bundle.externalBin` ships next to the app executable (tauri-build also copies it to `target/debug/` for dev — **a cargo build fails if it isn't staged**, so run `npm run cli:build` before a bare `cargo check`). Settings → System → "Agent CLI (ow)" (`get_cli_status` / `install_cli` in `cli_cmds.rs`) shows installed vs bundled version and skill freshness (installed SKILL.md compared to the one compiled into the app) and runs the bundled binary's `self-install` windowlessly: copy to the bin dir, add it to the user PATH (Windows registry, `REG_EXPAND_SZ` preserved), write the skills. `npm run cli:install` is the headless equivalent.
...
- **Session PR lifecycle (v1: detect / view / merge — no in-app PR creation):** (design: `docs/conductor-inspired-features-brainstorm-2026-07.md`). Backend: `fetch_branch_pr` (`gh pr view <branch> --json …` incl. `statusCheckRollup`, normalized `GitHubPrStatus`/`GitHubPrCheck`; "no PR" gh errors → `None`) and `merge_github_pr` (`gh pr merge --squash|--merge|--rebase`, deliberately no `--delete-branch` — cleanup is manual), both via the generic pinned `run_gh` helper; `get_git_default_branch` (git_cmds) exposes the remote default branch. Frontend: `stores/sessionPrs.ts` — per-session detection (only for sessions on a **non-default** branch of a GitHub-linked repo; 60s staleness, force-refresh when a turn ends since the agent may have just created the PR), merge (strategy remembered per repo via `RepoConfig.last_merge_strategy`), and panel-open state; a `SessionPrSummary` is mirrored onto `SdkSession.pr` (persisted) for badges, and the PR dock's open/closed flag is persisted to `SdkSession.prPanelOpen` (seeded back into the store by `sessionPrs.rehydrateFromSessions()` on launch) so the dock reopens after restart. **Detection is worktree-scoped:** sessions sharing a `cwd + branch` scope are looking at the same PR, so a fetch/refresh/cleanup by one is propagated to all siblings and staleness is scope-wide (no duplicate concurrent fetches, no diverging status across sessions in one worktree). When an **open** PR first appears for a scope, the dock panel auto-opens once per scope (gated by `auto_open_session_panels`, default on; see below). UI: clickable PR badge (icon + #number, state-colored: green open / gray draft / purple merged / red closed) right of the branch in `SdkSessionHeader` (opens the panel) and `SessionListItem`; `sdk/PrPanel.svelte` above the prompt input — fixed header (title/state/review chips) + **Overview | Diff** tabs + pinned merge/cleanup footer; Overview holds the collapsible markdown **description**, head→base + diffstat, CI checks with links, strategy picker + Merge, 15s polling while open. The **Diff** tab (`sdk/PrDiffView.svelte`) lazily loads the PR's unified diff via `fetch_pr_diff` (`gh pr diff <n>`, backend-capped at 2 MB with a `truncated` flag) the first time it's opened, caches it on the entry keyed by a diffstat **signature** (so the 15s PR poll re-fetches it only after a push), and renders it as collapsible per-file cards with old/new line numbers (`utils/gitDiff.ts` `parseUnifiedDiff`; files auto-collapse past ~1500 lines, per-file render cap 2000 lines). Plus a built-in green quick-action chip in `SdkQuickActions` ("Commit, push, create PR, merge from origin/<default> if needed") shown for non-default-branch sessions — the agent-driven creation path. **Post-merge cleanup (guided, one click each):** the merged banner offers "Delete branch & worktree" → `cleanup_merged_branch` (`GitManager::cleanup_merged_branch`) which **refuses unless safe** (no uncommitted changes, no unpushed commits, never the default branch, worktree must be a registered non-main one) then removes the worktree, deletes the local branch (`-D`, safe because of the unpushed check; checks out the default branch first for non-worktree sessions) and best-effort deletes the remote branch — plus "Archive session" (`sdkSessions.closeSession`). Button disabled while any session in the same cwd scope is busy; after cleanup, PR refreshes for that session stop (the cwd is gone).
...
- **Backend** (`src-tauri/src/validation/{types,prompts,ship,executor}.rs`, `commands/validation_cmds.rs`, `config/validation.rs`): `ValidationManager` (runs map, one tokio task per run). Events: `validation-update-${runId}` (full run snapshot on every change), `validation-log-${runId}`, `validation-fix-request-${runId}`, `validation-agent-activity-${runId}` (live per-tool-call/text agent activity; client-only bounded feed, not part of the snapshot). Commands: `validation_start_run` (returns backend-generated runId), `validation_get_run` (post-attach resync), `validation_respond` (approve|skip|fix), `validation_execute_ship`, `validation_fix_done`/`validation_fix_failed`, `validation_cancel`. Auto-fix limits per step from config (review 0 → always gates; test/lint/ci self-fix within bounds). **One producer pass per step:** agent-produced steps (review, docs, and test/lint with no configured command — `RunCtx::is_single_pass`) run their agent exactly once per run, so a fix is never re-reviewed; the fixed findings are retired and the step passes (or re-parks the same gate with any blocking findings the user left unselected). Only command-backed test/lint re-run the step after a fix round and park as `fix_review` with a `git diff` of what changed. Prompts (`prompts.rs`) carry the severity/action taxonomies, a do-NOT-flag list, BEGIN/END data-framing of intent/findings, and the **intent-conformance clause** (removed-required / added-forbidden behavior → mandatory ask-user finding).
- **Review agents** are headless one-shots through the sidecar (`OutboundMessage::ValidationAgent` → sidecar `validation_agent` handler, cloned from the repo-description generator): `query()` with `settingSources: []`, role-specific `submit_*` MCP tools (`submit_review`/`submit_verification`/`submit_evidence`/`submit_housekeeping`), a `canUseTool` Bash guard (git-only for review/verify/docs; open for evidence/lint), always a fresh session (the sidecar still accepts `resumeSessionId`, but the executor never sends it — one review pass per run), results via `validation-agent-result/error-${id}` `{ structured, transcript, sdkSessionId, usage }`, streamed progress via `validation-agent-progress-${id}` (one event per tool call / text block — drives the UI activity feed and the executor's **activity-aware timeout**: fixed 10-min idle window reset on every progress event + configurable overall cap `agent_timeout_minutes`, default 60). Claude provider only. Reviewer model/effort chosen per run ("Session model" is resolved to a concrete model id frontend-side); optional adversarial verify (each error finding gets a refute-me one-shot; refuted findings dropped).
- **Simplify step** (first, opt-in, not in default_steps): one autonomous headless agent running the Claude Code "Simplify" skill prompt **verbatim** (`prompts::simplify_prompt` — gather diff → 4 parallel cleanup subagents (reuse/simplification/efficiency/altitude) → apply fixes itself). Deliberately NO findings, NO gates, NO fix loop — the agent edits the working tree directly and its closing summary becomes the step note (transcript in the Steps drawer); agent error fails the run. When the tree was clean going in and the checkout is on a feature branch, the prompt also tells the agent to commit and push its own fixes. **Provider-agnostic by model**: the per-run "Simplify model" (any active provider's model, or "Session model"; `RunOptions.simplify_model`, falls back to reviewer model) is routed by the sidecar — Claude models use the `validation_agent` query() rail with an edit-capable role (`submit_simplify` summary tool, `acceptEdits`, Edit/Write/Task allowed, no Bash guard), GPT/Codex models run a headless **`danger-full-access` Codex thread** (`runValidationCodexThread`, no submit tools — final agent message is the summary, item events feed the activity/idle-timeout rail). Simplify is the only validation role on that sandbox, for the same reason interactive Auto-approve Codex sessions use it (`CodexPermissionMode`): under `workspace-write` the Windows sandbox leaves `.git` unwritable (`git add` → "Unable to create `.git/index.lock`") and breaks git's schannel TLS, and `approvalPolicy: "never"` gives no approval path out. Read-only roles stay `read-only`; evidence/lint stay `workspace-write`. (`stores/validation.ts` + `utils/validationFix.ts`): on `validation-fix-request` the store composes a fix prompt (double-check legitimacy first; root-cause; fix forward; ONE focused verification, no full suites) and routes it per the run's client-only `fixTarget` — the session's own agent (wait for idle → `sdkSessions.sendPrompt`) or a **fresh session** cloned from the origin (provider/model/effort/account) in the run's exact cwd, whose first turn is the fix prompt prepended with the run's intent + cwd context; either way the fixer session is watched and the outcome reported via `validation_fix_done`/`_failed`. Cancel never interrupts an in-flight session turn.
...
- **Frontend**: `stores/validation.ts` (runId-keyed store, full-snapshot `applySnapshot`, auto-selects auto-fix findings at gates, per-repo last-used run options in localStorage), `utils/validationIntent.ts` (rich intent: aiMetadata + ALL user messages verbatim, ~6000-char cap). **Restart persistence:** `SdkSession.validation` mirrors the compact badge summary, and `SdkSession.validationRun` mirrors the **full** run snapshot (steps/gate/outcome/PR link + dock **panel-open** state) via `persistRun`, so the whole panel — not just the badge — survives restart. On launch `validation.rehydrateFromSessions()` (called from the main layout after `loadSessionsFromDisk`) restores each run into the store seeded read-only (`ValidationRunView.detached`), then probes `validation_get_run`: if the backend still owns the run (a webview reload, not a full process restart) it reattaches live listeners and clears `detached`. Otherwise the backend forgot the run with the process that died, and the frontend **hands the snapshot back** (`validation.resume` → `validation_resume_run`) so it becomes actionable again: `ValidationManager::resume_run` re-registers the run, skips every step already in a terminal status (`StepStatus::is_terminal`) and passes the persisted gate to the first step that isn't — findings/fix_review gates re-park verbatim (no re-review), ship keeps the user's proposal, `ci_failure` re-parks; a step that was mid-flight has no recoverable state and re-runs from the top. Runs parked at a **gate** resume automatically during rehydration (re-parking costs nothing); a run interrupted mid-step stays **detached** behind an explicit "Resume run" button in the panel banner, since resuming re-runs that step's agent. Finished runs are always detached read-only history. `dismiss` clears `validationRun` so a dismissed run doesn't rehydrate. Both `SdkSession.validation`/`validationRun` (and `pr`/`prPanelOpen`) are opaque `serde_json::Value`/bool fields on the Rust `PersistedSdkSession` (frontend-owned schema) — required, since that struct is typed and silently drops fields it doesn't declare.
- **UI**: "Validate" button in `SdkSessionHeader` (not dev-gated; idle + real cwd) → `sdk/ValidationStartPopover.svelte` (step checkboxes, reviewer model/effort, adversarial verify; seeded localStorage → repo → settings); `sdk/ValidationPanel.svelte` in the pinned slot above the prompt input — **hybrid capped layout**: always-visible chrome (header with expand toggle, stepper, live agent-activity line, outcome banner) over a single tabbed scroll body capped at ~34vh (expand → ~72vh) with tabs Gate (auto-selected on gate; findings as stacked title-first rows, no file column) / Steps (proof lines with expandable output, risk chip, evidence artifacts, transcript/diff drawers) / Activity (timestamped feed) / Log (auto-selected on failure); gate actions (Fix selected + fix-target select "this session / a new session" / Approve / Skip) pinned below the body; four gate kinds: findings / fix_review / ship / ci_failure; status badge in `SessionListItem`; Settings → Validation (`settings/ValidationTab.svelte`); per-repo test/lint commands + review guidelines + default steps in `RepositoryView`'s Repository Settings. A closed panel auto-reopens on a new gate or a terminal pass/fail — gated by `auto_open_session_panels` (default on; the same top-level setting that governs PR-panel auto-open, toggled in Settings → General).
