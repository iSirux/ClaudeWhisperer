          ...
          <div>
            ...
            <div class="p-3 bg-surface-elevated rounded-lg space-y-3">
              ...
              {#if totalCodex > 0}
                ...
                {#if ($usageStats.model_usage.codex_6_astra_sessions || 0) > 0}
                  <div class="flex items-center gap-3">
                    <div class="w-20 text-sm text-text-secondary">6 Astra</div>
                    <div class="flex-1 bg-border rounded-full h-2">
                ...
                {#if ($usageStats.model_usage.codex_56_sol_sessions || 0) > 0}
                  <div class="flex items-center gap-3">
                    <div class="w-20 text-sm text-text-secondary">5.6 Sol</div>
                    <div class="flex-1 bg-border rounded-full h-2">
                      <div class="bg-green-500 h-2 rounded-full" style="width: {getModelPercentage($usageStats.model_usage.codex_56_sol_sessions || 0, totalModelsAll)}%"></div>
                    </div>
                    <div class="w-10 text-right text-sm text-text-primary">{$usageStats.model_usage.codex_56_sol_sessions || 0}</div>
                  </div>
                {/if}
                {#if ($usageStats.model_usage.codex_56_terra_sessions || 0) > 0}
                  <div class="flex items-center gap-3">
                    <div class="w-20 text-sm text-text-secondary">5.6 Terra</div>
                    <div class="flex-1 bg-border rounded-full h-2">
                      <div class="bg-green-400 h-2 rounded-full" style="width: {getModelPercentage($usageStats.model_usage.codex_56_terra_sessions || 0, totalModelsAll)}%"></div>
                    </div>
                    <div class="w-10 text-right text-sm text-text-primary">{$usageStats.model_usage.codex_56_terra_sessions || 0}</div>
                  </div>
                {/if}
                {#if ($usageStats.model_usage.codex_56_luna_sessions || 0) > 0}
                  <div class="flex items-center gap-3">
                    <div class="w-20 text-sm text-text-secondary">5.6 Luna</div>
                    <div class="flex-1 bg-border rounded-full h-2">
                      <div class="bg-green-300 h-2 rounded-full" style="width: {getModelPercentage($usageStats.model_usage.codex_56_luna_sessions || 0, totalModelsAll)}%"></div>
                    </div>
                    <div class="w-10 text-right text-sm text-text-primary">{$usageStats.model_usage.codex_56_luna_sessions || 0}</div>
                  </div>
                {/if}
