<script lang="ts">
  ...
  import { getModelBadgeBgColor, getModelTextColor } from '$lib/utils/modelColors';
  ...
  async function generateRepoDescription(index: number) {
    ...
    try {
      const result = await invoke<RepoDescriptionResult>('generate_repo_description', {
        repoPath: repo.path,
        repoName: repo.name,
      });

      // Update the repo's description and keywords in settings separately
  ...
  function toggleModel(modelId: string) {
    ...
    if (isEnabled) {
      ...
      // Add the model
      settings.update(s => ({ ...s, enabled_models: [...currentEnabled, modelId] }));
    }
  }
      ...
      {#if activeTab === 'general'}
        <div class="space-y-4">
          ...
          <div class="border-t border-border pt-4 mt-4">
            ...
            <p class="text-xs text-text-muted mb-3">Select which models are available in the model selector and for hotkey cycling. At least one model must remain enabled.</p>
            <div class="space-y-2">
              {#each ALL_MODELS as model}
                {@const enabled = isModelEnabled(model.id)}
                {@const isOnlyEnabled = enabled && ($settings.enabled_models || []).length === 1}
                <button
                  class="w-full flex items-center justify-between p-3 rounded border-2 transition-all {enabled ? 'border-accent bg-accent/10' : 'border-border opacity-50'}"
                  class:cursor-not-allowed={isOnlyEnabled}
                  onclick={() => toggleModel(model.id)}
                  title={isOnlyEnabled ? 'At least one model must remain enabled' : model.title}
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div class="text-left">
                      <div class="flex items-center gap-2">
                        <span class="text-sm font-medium px-2 py-0.5 rounded {getModelBadgeBgColor(model.id)} {getModelTextColor(model.id)}">{model.label}</span>
                      </div>
                      <p class="text-xs text-text-muted mt-0.5">{model.title}</p>
                    </div>
                  </div>
                  {#if isOnlyEnabled}
      ...
      {:else if activeTab === 'whisper'}
        <div class="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onchange={(e) => {
                ...
                if (provider === 'Local') {
                  $settings.whisper.endpoint = 'http://localhost:8000/v1/audio/transcriptions';
                  $settings.whisper.model = 'Systran/faster-whisper-large-v3-turbo';
                  $settings.whisper.api_key = null;
                } else if (provider === 'OpenAI') {
                  $settings.whisper.endpoint = 'https://api.openai.com/v1/audio/transcriptions';
                  $settings.whisper.model = 'whisper-1';
                } else if (provider === 'Groq') {
                  $settings.whisper.endpoint = 'https://api.groq.com/openai/v1/audio/transcriptions';
            ...
            <p class="text-xs text-text-muted mt-1">
              ...
              {:else if $settings.whisper.provider === 'OpenAI'}
                Official OpenAI Whisper API - <a href="https://platform.openai.com/api-keys" class="text-accent hover:underline" target="_blank" rel="noopener noreferrer">Get API key</a>
              {:else if $settings.whisper.provider === 'Groq'}
                Fast inference with free tier - <a href="https://console.groq.com/keys" class="text-accent hover:underline" target="_blank" rel="noopener noreferrer">Get API key</a>
              {:else}
          ...
          <div>
            ...
            {:else}
              <input type="text" class="w-full px-3 py-2 bg-background border border-border rounded text-sm focus:outline-none focus:border-accent" bind:value={$settings.whisper.model} placeholder="whisper-1" />
            {/if}
          </div>
          ...
          {:else if whisperStatus === 'partial'}
            <p class="text-sm text-warning">Partial connection (transcription may still work)</p>
          {:else if whisperStatus === 'error'}
            <p class="text-sm text-error">Connection failed. Check your endpoint and API key.</p>
          {/if}

          {#if whisperTestResult}
            <div class="space-y-1 text-xs">
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full {whisperTestResult.health_ok ? 'bg-success' : 'bg-warning'}"></div>
                <span class="text-text-secondary">Health:</span>
                <span class="text-text-muted">{whisperTestResult.health_ok ? 'OK' : (whisperTestResult.health_error || 'N/A (some providers don\'t expose health endpoint)')}</span>
              </div>
              <div class="flex items-center gap-2">
          ...
          {#if $settings.whisper.provider === 'Local'}
            {@const dockerCommand = (() => {
              ...
              const imageTag = $settings.whisper.docker.compute_type === 'GPU' ? 'latest-cuda' : 'latest-cpu';
              parts.push(`fedirz/faster-whisper-server:${imageTag}`);

              return parts.join(' \\\n  ');
            })()}
            <div class="border-t border-border pt-4 mt-4">
              <label class="block text-sm font-medium text-text-secondary mb-3">Docker Setup</label>

              <!-- Compute Type Selection -->
              <div class="mb-4">
                <label class="block text-xs font-medium text-text-muted mb-2">Compute Type</label>
                <div class="flex gap-2">
                  <button
                    class="flex-1 px-3 py-2 text-sm rounded border-2 transition-all flex items-center justify-center gap-2 {$settings.whisper.docker.compute_type === 'CPU' ? 'border-accent bg-accent/10' : 'border-border'}"
                    onclick={() => settings.update(s => ({ ...s, whisper: { ...s.whisper, docker: { ...s.whisper.docker, compute_type: 'CPU' } } }))}
                  >
                  ...
                  <button
                    class="flex-1 px-3 py-2 text-sm rounded border-2 transition-all flex items-center justify-center gap-2 {$settings.whisper.docker.compute_type === 'GPU' ? 'border-accent bg-accent/10' : 'border-border'}"
                    onclick={() => settings.update(s => ({ ...s, whisper: { ...s.whisper, docker: { ...s.whisper.docker, compute_type: 'GPU' } } }))}
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <p class="text-xs text-text-muted mt-1.5">
                  {#if $settings.whisper.docker.compute_type === 'CPU'}
                    Good for laptops and systems without NVIDIA GPUs. Slower but works everywhere.
                  {:else}
                    Requires NVIDIA GPU with CUDA. Much faster transcription for desktops.
                  {/if}
                </p>
              </div>
              ...
              <div class="flex items-center justify-between mb-4">
                <div>
                  <label class="text-sm font-medium text-text-secondary">Auto-start with Docker</label>
                  <p class="text-xs text-text-muted">Container starts automatically when Docker Engine starts</p>
                </div>
                <input
                  type="checkbox"
                  class="toggle"
                  ...
                  onchange={(e) => {
                    ...
                    settings.update(s => ({ ...s, whisper: { ...s.whisper, docker: { ...s.whisper.docker, auto_restart: checked } } }));
                  }}
                />
              </div>
              ...
              <!-- Container Name -->
              <div class="mb-4">
                <label class="block text-xs font-medium text-text-muted mb-1">Container Name</label>
                <input
                  type="text"
                  class="w-full px-3 py-2 bg-background border border-border rounded text-sm focus:outline-none focus:border-accent font-mono"
                  ...
                  oninput={(e) => {
                    ...
                    settings.update(s => ({ ...s, whisper: { ...s.whisper, docker: { ...s.whisper.docker, container_name: value } } }));
                  }}
                  placeholder="whisper"
                />
              </div>
              ...
              <!-- Generated Command -->
              <div>
                <label class="block text-xs font-medium text-text-muted mb-2">Docker Command</label>
                <div class="relative group">
                  <pre class="p-3 bg-background border border-border rounded text-xs font-mono text-text-primary overflow-x-auto whitespace-pre-wrap break-all">{dockerCommand}</pre>
                  <button
        ...
        <div class="space-y-4">
          <div class="p-3 bg-surface-elevated rounded border border-border">
            ...
            <p class="text-xs text-text-muted">
              Use a lightweight LLM for auxiliary tasks like session naming, interaction detection, and note structuring.
              Supports Google Gemini, OpenAI, Groq, or local models (LM Studio, Ollama, etc.).
            </p>
          </div>
          ...
          <div>
            ...
            <p class="text-xs text-text-muted mt-1">
              {#if $settings.llm.provider === 'Gemini'}
                Free tier: 1,500 requests/day - <a href="https://aistudio.google.com/apikey" class="text-accent hover:underline" target="_blank" rel="noopener noreferrer">Get API key</a>
              {:else if $settings.llm.provider === 'OpenAI'}
                Paid API - <a href="https://platform.openai.com/api-keys" class="text-accent hover:underline" target="_blank" rel="noopener noreferrer">Get API key</a>
              {:else if $settings.llm.provider === 'Groq'}
                Free tier available - <a href="https://console.groq.com/keys" class="text-accent hover:underline" target="_blank" rel="noopener noreferrer">Get API key</a>
          ...
          {#if $settings.llm.provider !== 'Local'}
            <div class="border-t border-border pt-4">
              ...
              {#if geminiApiKeySet}
                <div class="flex items-center justify-between p-3 bg-success/10 border border-success/30 rounded">
                  <div class="flex items-center gap-2">
            ...
            <div class="border-t border-border pt-4">
              <div class="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p class="text-xs text-text-muted">Use for session naming and interaction detection</p>
                </div>
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    {#if $settings.llm.provider === 'Gemini'}
                      ...
                      <div class="flex items-center justify-between mb-3 p-3 bg-surface-elevated rounded border border-border">
                        <div>
                        ...
                        <input
                          ...
                          onchange={() => settings.update(s => ({ ...s, llm: { ...s.llm, auto_model: !s.llm.auto_model } }))}
                        />
                      </div>
                      ...
                      {#if $settings.llm.auto_model}
                        ...
                        <div class="mb-3">
                          <label class="block text-xs font-medium text-text-secondary mb-2">Priority</label>
                          <div class="flex gap-2">
                            <button
                              class="flex-1 py-2 px-3 rounded text-sm font-medium transition-all {$settings.llm.model_priority === 'speed' ? 'bg-accent text-white' : 'bg-surface-elevated text-text-secondary hover:bg-border'}"
                              onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, model_priority: 'speed' } }))}
                            >
                            ...
                            <button
                              class="flex-1 py-2 px-3 rounded text-sm font-medium transition-all {$settings.llm.model_priority === 'accuracy' ? 'bg-accent text-white' : 'bg-surface-elevated text-text-secondary hover:bg-border'}"
                              onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, model_priority: 'accuracy' } }))}
                            >
                              <div class="flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <p class="text-xs text-text-muted mt-2">
                            {#if $settings.llm.model_priority === 'speed'}
                              Prioritizes 2.5 Flash-Lite for faster responses, falls back to 2.5 Flash then 2.0 Flash
                            {:else}
                              Prioritizes 2.5 Flash for better quality, falls back to 2.5 Flash-Lite then 2.0 Flash
                            {/if}
                          </p>
                        </div>
                      {:else}
                        ...
                        <p class="text-xs text-text-muted mt-1">Gemini 2.5 Flash offers the best balance of speed and quality</p>
                      {/if}
                    {:else if $settings.llm.provider === 'OpenAI'}
                      <select
                        class="w-full px-3 py-2 bg-background border border-border rounded text-sm focus:outline-none focus:border-accent"
                    ...
                    {:else}
                      ...
                      />
                      <p class="text-xs text-text-muted mt-1">Enter the model name as expected by your endpoint</p>
                    {/if}
                  </div>
                  ...
                  {:else if geminiStatus === 'error'}
                    <p class="text-sm text-error">Connection failed: {geminiTestResult?.error || 'Unknown error'}</p>
                  {/if}

                  <div class="border-t border-border pt-4">
                    <h3 class="text-sm font-medium text-text-primary mb-2">Features</h3>
                    <p class="text-xs text-text-muted mb-3">Choose which LLM-powered features to enable. Each feature uses API calls against your provider quota.</p>
                    <div class="space-y-2">
                      <button
                        class="w-full flex items-center justify-between p-3 rounded border-2 transition-all text-left {$settings.llm.features.auto_name_sessions ? 'border-accent bg-accent/10' : 'border-border hover:border-border/80'}"
                        onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, features: { ...s.llm.features, auto_name_sessions: !s.llm.features.auto_name_sessions } } }))}
                      >
                        <div class="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p class="text-xs text-text-muted mt-0.5">Generate descriptive names and categories for sessions based on the conversation content</p>
                          </div>
                        </div>
                      </button>

                      <button
                        class="w-full flex items-center justify-between p-3 rounded border-2 transition-all text-left {$settings.llm.features.detect_interaction_needed ? 'border-accent bg-accent/10' : 'border-border hover:border-border/80'}"
                        onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, features: { ...s.llm.features, detect_interaction_needed: !s.llm.features.detect_interaction_needed } } }))}
                      >
                        <div class="flex items-center gap-3">
                          <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors flex-shrink-0"
                          ...
                          <div>
                            ...
                            <p class="text-xs text-text-muted mt-0.5">Analyze Claude's responses to detect when your input is required (questions, approvals, decisions)</p>
                          </div>
                        </div>
                      </button>

                      <button
                        class="w-full flex items-center justify-between p-3 rounded border-2 transition-all text-left {$settings.llm.features.clean_transcription ? 'border-accent bg-accent/10' : 'border-border hover:border-border/80'}"
                        onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, features: { ...s.llm.features, clean_transcription: !s.llm.features.clean_transcription } } }))}
                      >
                        <div class="flex items-center gap-3">
                          <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors flex-shrink-0"
                          ...
                          <div>
                            ...
                            <p class="text-xs text-text-muted mt-0.5">Fix homophones, technical terms, and punctuation in voice transcriptions before sending to Claude</p>
                          </div>
                        </div>
                      </button>

                      <button
                        class="w-full flex items-center justify-between p-3 rounded border-2 transition-all text-left {$settings.llm.features.recommend_model ? 'border-accent bg-accent/10' : 'border-border hover:border-border/80'}"
                        onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, features: { ...s.llm.features, recommend_model: !s.llm.features.recommend_model } } }))}
                      >
                        <div class="flex items-center gap-3">
                          <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors flex-shrink-0"
                          ...
                          <div>
                            ...
                            <p class="text-xs text-text-muted mt-0.5">Automatically choose the best Claude model (Haiku/Sonnet/Opus) based on prompt complexity to optimize cost</p>
                          </div>
                        </div>
                      </button>

                      <button
                        class="w-full flex items-center justify-between p-3 rounded border-2 transition-all text-left {$settings.llm.features.auto_select_repo ? 'border-accent bg-accent/10' : 'border-border hover:border-border/80'}"
                        onclick={() => settings.update(s => ({ ...s, llm: { ...s.llm, features: { ...s.llm.features, auto_select_repo: !s.llm.features.auto_select_repo } } }))}
                      >
                        <div class="flex items-center gap-3">
                          <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors flex-shrink-0"
                          ...
                          <div>
                            ...
                            <p class="text-xs text-text-muted mt-0.5">Automatically select the best repository based on prompt content. Requires repo descriptions to be generated.</p>
                          </div>
                        </div>
                      </button>
                    </div>
                    ...
                    {#if $settings.llm.features.auto_select_repo}
                      <div class="mt-3 ml-8 pl-3 border-l-2 border-border space-y-3">
                        <div>
                          <label class="block text-sm font-medium text-text-secondary mb-1">Minimum Confidence for Auto-Select</label>
                          <p class="text-xs text-text-muted mb-2">Only auto-select repos when LLM confidence meets this threshold</p>
                          <select class="w-full px-3 py-2 bg-background border border-border rounded text-sm focus:outline-none focus:border-accent" bind:value={$settings.llm.min_auto_select_confidence}>
                        ...
                        <div class="flex items-center justify-between">
                          <div>
                            ...
                            <p class="text-xs text-text-muted">Claude will question the repo selection if it seems wrong</p>
                          </div>
                          <input type="checkbox" class="toggle" bind:checked={$settings.llm.confirm_repo_selection} />
                        </div>
                      </div>
                    {/if}
                  </div>
          ...
          {#if $settings.llm.provider === 'Gemini'}
            <div class="border-t border-border pt-4 mt-4">
              <h3 class="text-sm font-medium text-text-secondary mb-2">Free Tier Limits</h3>
              <div class="text-xs text-text-muted space-y-1">
                <p><strong>Gemini 2.0 Flash:</strong> 15 RPM, 1M TPM, 1,500 requests/day</p>
                <p><strong>Gemini 2.5 Flash:</strong> 10 RPM, 250K TPM, 250 requests/day</p>
                <p><strong>Gemini 2.5 Flash-Lite:</strong> 15 RPM, 250K TPM, 1,000 requests/day</p>
                <p class="mt-2 text-text-muted">Limits reset at midnight Pacific Time. No credit card required.</p>
              </div>
            </div>
          {:else if $settings.llm.provider === 'Groq'}
            <div class="border-t border-border pt-4 mt-4">
              ...
              <div class="text-xs text-text-muted space-y-1">
                ...
                <p class="mt-2 text-text-muted">Generous free tier with no credit card required.</p>
              </div>
            </div>
          {:else if $settings.llm.provider === 'Local'}
            <div class="border-t border-border pt-4 mt-4">
              ...
              <div class="text-xs text-text-muted space-y-1">
                <p><strong>LM Studio:</strong> Download from <a href="https://lmstudio.ai" class="text-accent hover:underline" target="_blank">lmstudio.ai</a>, load a model, and start the server</p>
                <p><strong>Ollama:</strong> Install from <a href="https://ollama.ai" class="text-accent hover:underline" target="_blank">ollama.ai</a> and run <code class="bg-background px-1 rounded">ollama serve</code></p>
                <p class="mt-2 text-text-muted">Recommended models: Llama 3.1 8B, Mistral 7B, Qwen 2.5</p>
              </div>
            </div>
          {/if}
        </div>
          ...
          <div class="space-y-3">
            {#each $settings.repos as repo, index}
              <div class="p-3 bg-surface-elevated rounded space-y-2">
                <div class="flex items-start gap-2">
                  ...
                  <div class="flex gap-1 shrink-0">
                    ...
                    <button
                      ...
                      disabled={!$settings.llm.enabled || generatingDescriptionForIndex !== null}
                      title={!$settings.llm.enabled ? 'Enable LLM integration in LLM settings to generate descriptions' : 'Generate description from CLAUDE.md or README'}
                    >
                    ...
                    <!-- Remove button -->
                    <button class="p-1.5 text-text-muted hover:text-error transition-colors rounded hover:bg-border" onclick={() => removeRepo(index)}>
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Description and keywords area -->
                {#if repo.description || repo.keywords?.length}
                  <div class="text-xs bg-background/50 p-2 rounded border border-border/50 space-y-2">
                    {#if repo.description}
                    ...
                    {#if repo.keywords?.length}
                      <div class="flex flex-wrap gap-1">
                        {#each repo.keywords as keyword}
                          <span class="px-1.5 py-0.5 bg-accent/10 text-accent text-[10px] rounded">{keyword}</span>
                        {/each}
                      </div>
                    {/if}
                  </div>
                {:else}
                  <div class="text-xs text-text-muted italic">
                    No description. {#if $settings.llm.enabled}Click ✨ to generate from CLAUDE.md/README.{:else}Enable LLM integration to auto-generate.{/if}
                  </div>
                {/if}
              </div>
            {/each}
