<script lang="ts">
  ...
  function openWhisperSettings() {
    window.dispatchEvent(new CustomEvent('open-settings', { detail: { tab: 'whisper' } }));
  }

  function openAudioSettings() {
    window.dispatchEvent(new CustomEvent('open-settings', { detail: { tab: 'audio' } }));
  }
...
<div class="start-panel flex flex-col h-full">
  <header class="flex items-center justify-between px-4 py-3 border-b border-border">
    <h2 class="text-lg font-semibold text-text-primary">Start</h2>
    <button
  ...
  <div class="flex-1 flex items-center justify-center p-8 overflow-y-auto">
    <div class="max-w-lg w-full space-y-8">
      <div class="text-center">
        ...
        <p class="text-text-secondary">Let's make sure everything is set up correctly.</p>
      </div>

      <!-- Microphone Selection -->
      <div class="bg-surface-elevated border border-border rounded-lg p-6 overflow-hidden">
        ...
        {#if audioDevices.length === 0 && !loadingDevices}
          <p class="text-xs text-text-muted mt-2">No microphones found. Please connect a microphone and click Refresh.</p>
        {/if}
      </div>
      ...
      <div class="bg-surface-elevated border border-border rounded-lg p-6">
        <div class="flex items-center gap-3 mb-4">
          <div class="p-2 rounded-lg {whisperStatus === 'success' ? 'bg-success/10' : whisperStatus === 'error' ? 'bg-error/10' : 'bg-accent/10'}">
          ...
          <div class="flex-1">
            <h3 class="text-lg font-medium text-text-primary">Whisper Connection</h3>
        ...
        {#if whisperStatus === 'error'}
          <div class="mt-4 p-3 bg-error/10 border border-error/20 rounded text-sm">
            <p class="text-error font-medium mb-1">Whisper server not available</p>
            <p class="text-text-secondary text-xs">
              Make sure your Whisper server is running at <code class="px-1 py-0.5 bg-background rounded">{$settings.whisper.endpoint}</code>
            </p>
          </div>
        {/if}
      </div>
      ...
      <div class="flex items-center justify-center gap-6 text-sm">
        <div class="flex items-center gap-2">
          <div class="w-2 h-2 rounded-full" class:bg-success={audioDevices.length > 0} class:bg-error={audioDevices.length === 0}></div>
          <span class="text-text-muted">Microphone</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-2 h-2 rounded-full" class:bg-success={whisperStatus === 'success'} class:bg-error={whisperStatus === 'error'} class:bg-text-muted={whisperStatus === 'idle' || whisperStatus === 'testing'}></div>
